...
async function chargerListingsRecus() {
  ...
  if (error) {
    tbody.innerHTML = '<tr><td colspan="5" class="py-8 text-center text-red-600">Impossible de charger l\'historique.</td></tr>';
    return;
  }
  if (!data || !data.length) {
    tbody.innerHTML = '<tr><td colspan="5" class="py-8 text-center text-slate-500">Aucun listing reçu pour le moment.</td></tr>';
    return;
  }
